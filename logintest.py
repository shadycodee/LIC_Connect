...
def get_usertype(student_id):
    ...
    query = "SELECT user_type FROM students WHERE studentid = %s"
...
def login():
    ...
    usertype = get_usertype(student_id)
...
def librarian_window():
    window = tk.Tk()
    window.title("Logged In")
    window.geometry("300x150")

    logout_button = tk.Button(window, text="Logout", command=window.destroy)
    logout_button.pack()
